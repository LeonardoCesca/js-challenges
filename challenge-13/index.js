...
(function () {
    ...
    */
    // ?
    let clone = brasil.slice();

    let newSul = clone.splice(2, 3);
    ...
    */
    ...
    brasil.every((estado) => {
        console.log(estado && estado > 7 ? 'Sim, todos os estados tem mais de 7 letras!' : 'Nem todos os estados tem mais de 7 letras!');
    })
   ...
   let result;
   brasil.some((estado) => {
       result = estado && estado === 'Ceará' ? 'Ceará está incluído!' : 'Ceará não foi incluído :(';
   })
    console.log( '\nCeará está incluído em `brasil`? ' + result );
    // ?
    ...
    */
    console.log( '\nEstados com ID par:' );
    let filter = map.filter(estado => console.log(estado.id % 2 === 0));  
    // ?
    console.log(filter)
